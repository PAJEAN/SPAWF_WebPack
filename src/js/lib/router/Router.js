...
/** Store **/
import { StoreRouterManager } from 'JS/store';
    ...
    navigate(hash) {
        ...
        if (matched_route) {            
            if (matched_route.roles.length > 0) {
                if (StoreRouterManager.is_auth && StoreRouterManager.is_auth === true && StoreRouterManager.roles && matched_route.roles.some((el) => StoreRouterManager.roles.includes(el))) { // Interception between state.roles and matched_roles.
                    navigate = true;
                }
            }
            else {
